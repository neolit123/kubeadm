...
func TestParseEntries(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			totalFields: totalFieldsUser,
			expectedEntries: []*entry{
				&entry{name: "foo", id: 100, gid: 101, shell: "/bin/bash"},
				&entry{name: "bar", id: 102, gid: 103},
			},
		},
		{
			...
			expectedEntries: []*entry{
				&entry{name: "foo", id: 100, userNames: []string{"bar", "baz"}},
				&entry{name: "bar", id: 101, userNames: []string{"baz"}},
			},
		},
	}
...
func TestID(t *testing.T) {
	...
	ok, id := m.ID("foo")
	if !ok {
		t.Fatalf("expected ID() to return ok for a known entry")
	}
	if *id != 101 {
	...
	ok, _ = m.ID("bar")
	if ok {
		t.Fatalf("expected ID() !ok for unknown entry")
	}
}

func TestAddUsersAndGroups(t *testing.T) {
	const (
	...
	defer close()
	users, groups, err := AddUsersAndGroups(fileLoginDef, filePasswd, fileGroup)
	if err != nil {
		t.Fatalf("AddUsersAndGroups failed: %v", err)
	}
	if expectedUsers != users.String() {
		t.Fatalf("expected users: %q, got: %q", expectedUsers, users.String())
	}
	if expectedGroups != groups.String() {
		t.Fatalf("expected groups: %q, got: %q", expectedGroups, groups.String())
	}
}
...
func TestRemoveUsersAndGroups(t *testing.T) {
	...
	fileGroup, close := writeTempFile(t, group)
	defer close()
	if err := RemoveUsersAndGroups(filePasswd, fileGroup); err != nil {
