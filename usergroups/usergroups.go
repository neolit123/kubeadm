...
// It returns false as the first returned argument if no such entry exists.
func (u *EntryMap) ID(name string) (bool, *int64) {
...
// String convers a UIDAndGID object to readable string for unit testing.
...
var (
	...
	groupsToCreateSpec = []*entry{
		{name: "kubeadm-etcd", userNames: []string{"kubeadm-etcd"}},
		{name: "kubeadm-kas", userNames: []string{"kubeadm-kas"}},
		{name: "kubeadm-kcm", userNames: []string{"kubeadm-kcm"}},
		{name: "kubeadm-ks", userNames: []string{"kubeadm-ks"}},
		{name: "kubeadm-sa-key-readers", userNames: []string{"kubeadm-kas", "kubeadm-kcm"}},
	}
...
// parseEntries can be used a to parse an /etc/passwd or /etc/group file as their format is similar.
	...
	for i, line := range lines {
		...
		if len(fields) != totalFields {
			return nil, errors.Errorf("entry must have %d fields separated by ':', "+
				"got: %d at line %d", totalFields, len(fields), i)
		}
...
// AddUsersAndGroups adds the managed users and groups to the files specified
...
func AddUsersAndGroups(pathLoginDef, pathUsers, pathGroups string) (*EntryMap, *EntryMap, error) {
	// Read and parse /etc/login.def.
...
// RemoveUsersAndGroups removes the managed users and groups from the files specified
// by pathUsers and pathGroups.
func RemoveUsersAndGroups(pathUsers, pathGroups string) error {
	// read /etc/passwd
	fUsers, close, err := openFileWithLock(pathUsers)
	if err != nil {
		return err
